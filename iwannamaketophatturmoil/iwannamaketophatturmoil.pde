...
boolean textures = true;//not working right now sadly
      ...
      if (!((keys[0]>1)&&(keys[1])>1)) {
        if (keys[0]>1) {
          ...
          if (keys[3]==2) {
            ...
            animNum=1;
            if (!snd[sndLookup[0]].isPlaying())snd[sndLookup[0]].play();
          }
        } else if (keys[1]>1) {
          if (super.getVelocityX()<-200)super.setVelocity(super.getVelocityX()+100, super.getVelocityY());//super.addForce(5000,0);
          else super.setVelocity(200, super.getVelocityY());
...
void makeLevel() {
  try {
    ...
    if (fileType=='3') {
      ...
      if (fileType!='1') {
        String[]s = split(new String(subset(map, 0, map.length-33-map[map.length-26])), (char)0);
        loadTextures(subset(s, s.length-map[map.length-25]));
        makeEnemies(int(subset(map, lWidth*lHeight)));
      }
  ...
  case '1':
    ...
    case 3:
      ...
      for (int i=converted.length; i>0; i--) {
        converted = (byte[])splice(converted, (byte)0xFF, i);
        converted = (byte[])splice(converted, (byte)0, i);
      }
